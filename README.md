...
produce these characters through a combination of `RALT` (`AltGr`) + alphas.
The rest of the layout is similar to a regular ANSI layout.

For the terminal emulator, I use [Ghostty](https://ghostty.org/) and to enable
regular `Alt` / `Opt` usage in terminal, make sure to set macos-option-as-alt =
left, otherwise the key will be used exclusively for EurKEY combinations.
...
The keymap drawing is generated using
[keymap-drawer](https://github.com/caksoylar/keymap-drawer). use `make draw` to
re-generate the keymap drawing.
